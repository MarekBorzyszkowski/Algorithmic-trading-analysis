...
predicted_vals = predictions_results['predictions'][algorithm_name]
signals = []
for i in range(len(predicted_vals)):
    signal = BUY if predicted_vals[i] > test_data_close[i] \
        else SELL if predicted_vals[i] < test_data_close[i] \
        else KEEP
    signals.append(signal)
predictions_results['predictions']['signals'] = signals

with open(f"{newpath}/results/{algorithm_name}_predictions.json", "w") as file:
    json.dump(predictions_results, file, indent=4)
