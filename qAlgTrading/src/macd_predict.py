...
algorithm_result['MACD_Diff'] = algorithm_result['MACD'] - algorithm_result['MACD_SIGNAL']
algorithm_result['Bullish_Run_Start'] = (algorithm_result['MACD'] > algorithm_result['MACD_SIGNAL']) & (
            algorithm_result['MACD'].shift(1) <= algorithm_result['MACD_SIGNAL'].shift(1))
algorithm_result['Bearish_Run_Start'] = (algorithm_result['MACD'] < algorithm_result['MACD_SIGNAL']) & (
            algorithm_result['MACD'].shift(1) >= algorithm_result['MACD_SIGNAL'].shift(1))
# Identify bullish and bearish crossover points
algorithm_result['Bullish_Crossover'] = (algorithm_result['MACD'] > algorithm_result['MACD_SIGNAL']) & (
            algorithm_result['MACD'].shift(1) <= algorithm_result['MACD_SIGNAL'].shift(1))
algorithm_result['Bearish_Crossover'] = (algorithm_result['MACD'] < algorithm_result['MACD_SIGNAL']) & (
            algorithm_result['MACD'].shift(1) >= algorithm_result['MACD_SIGNAL'].shift(1))

results_presenter = ResultPresenter()
results_presenter.plot_MACD(algorithm_result, title=f"{present_name} - {algorithm_name}", component_name=component_name,
                            with_save=True, present_name_component=present_name)
